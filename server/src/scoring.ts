 ...
 */
export function calculateAdoptionScoreFormulaB(params: {
  qualityMultiplier: number;   // 0.6 - 2.0
  eventMultiplier: number;     // 1.0 - 3.0
}): number {
 ...
 */
export function getLeaderboardScore(totalPoints: number, reputation: number): number {
  return Math.floor(totalPoints * (1 + reputation / 100));
}

/** Legacy: kept for any callers that still use Formula A-style params */
export function calculateAdoptionScore(params: {
  basePoints: number;
  qualityMultiplier: number;
  diversityBonus: number;
  streakBonus: number;
  eventMultiplier: number;
  antiGrindFactor: number;
}): number {
  return Math.floor(
    params.basePoints * params.qualityMultiplier * params.diversityBonus * params.streakBonus * params.eventMultiplier * params.antiGrindFactor
  );
}
 ...
 */
export function getQualityMultiplier(options: {
  randomVariance?: number;      // 0.8 - 1.2
  eventRequirementMet?: boolean;
}): number {
 ...
 */
export function getDiversityBonus(uniquePetTypesAdoptedToday: number): number {
 ...
 */
export function getStreakBonus(perfectDaysStreak: number): number {
 ...
 */
export function getEventMultiplier(participatingInEvent: boolean, tier?: 'participation' | 'top3' | 'top2' | 'top1'): number {
  if (!participatingInEvent) return 1.0;
  switch (tier) {
 ...
 */
export function getAntiGrindFactor(adoptionsThisSession: number, adoptionsToday: number): number {
